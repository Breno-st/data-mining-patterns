...
def wracc(dataset):
        ...
        for i in range(0, len(result_prefix_closed)):
            supPositif = 0
            supNegatif = 0
...
def PrefixSpan_Sup_Clos_Wracc(*args):
    ...
    higher_recurr = np.amax(result_prefix_recur_closed_)

    counter = 0
    while(counter < k and higher_recurr > 0):

        # Get item with same recurrence
        freq_prefixes = get_recurr_prefix(result_prefix_closed_, result_prefix_recur_closed_, higher_recurr)
        closed_list(freq_prefixes)
        ...
        if len(freq_prefixes) > 0:
            ...
            for freq_prefix in freq_prefixes:
                ...
                st = st + str(np.round(higher_recurr,5))
                print(st)
...
#     PrefixSpan_Sup_Clos_Wracc(pos_filepath, neg_filepath, k)
